...
describe('Testing HashTable', () => {
  it('Should be able to add a key/value pair to the hashtable', () => {
    ...
    expect(hashTable.map[hashTable.hash('Antoine')].head.value['Antoine']).toStrictEqual('Dev');
  });
  ...
  it('Should be able to handle a collision in the hashtable', () => {
    ...
    expect(hashTable.map[hashTable.hash('Antoine')].head.next.value).toStrictEqual({ Antoine: 'Dev' });
  });
  ...
  it('Should be able to retrieve a value from a bucket within the hashtable that has a collision', () => {
    ...
    expect(hashTable.map[hashTable.hash('Antoine')].head.value['Antoine']).toStrictEqual('Engineer');
    expect(hashTable.map[hashTable.hash('Antoine')].head.next.value['Antoine']).toStrictEqual('Dev');
  });

  it('Should successfully hash a key to an in-range value', () => {
    const hashTable = new HashTable(1024);

    expect(hashTable.hash('Antoine')).toStrictEqual(2);
  });
